...
$ptdProv 	= $_POST['ptdProv'];
	...
	*/	
	...
	$sql1  = "SELECT * FROM pedido_temporal_detalle WHERE paisID = $paisID and ptID = ".$ptID." and ptdValor > 0 and ptdProv = ".$ptdProv." group by ptdRes";
	...
	if($usuario){
		foreach ($usuario as $u) {
			...
			$ptdVM  = $u['ptdVM'];
			$tipousuario = get_usertipo($ptdVM);
			
			if($tipousuario==1){
				$mailresponsable = get_user_mail($ptdVM);
			}else{
				$mailresponsable = get_user_mail($u['ptdRes']);
			}
		    ...
		    if($paisID==7){
				$message .= '<h3>Cota&ccedil;&atilde;o de Pedido N&ordm;'.$ptID.'</h3>';
		    }else{
				$message .= '<h3>Cotizaci&oacute;n Pedido N&ordm; '.$ptID.'</h3>';
		    } 			
			$sql  	= "SELECT count(*) as Total, ptID, ptdTS FROM pedido_temporal_detalle WHERE paisID = $paisID and ptID = ".$ptID." and ptdValor > 0 and ptdProv = ".$ptdProv." and ptdRes = ".$usuId;		
			...
			if($resultado){
				foreach ($resultado as $r0) {
					...
					$sql2  		= "SELECT *  FROM pedido_temporal_detalle WHERE paisID = $paisID and ptID = $ptID and ptdValor > 0 and ptdProv = ".$ptdProv." and ptdRes = ".$usuId;
